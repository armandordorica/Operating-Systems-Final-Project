...
int getDate( char date[] ){
	...
	time ( &current_time );
	curtimeinfo = localtime ( &current_time );

	int curd = curtimeinfo->tm_mday;
	int curm = curtimeinfo->tm_mon + 1;

	//printf("Time is %s", asctime(&tm));
	//printf("%d %d %d %d %d %d", curd+14 , d, d-curd, w, curm, m);

	if ((curd + 14) >=  d && curm == m && curd <= d){
		if(w != 0 && w != 6){
			if ((d-curd) >= 7){
				weekday = 20 + w;
				return weekday;
